...
CAN_frame BYD_250 = {
    ...
    .DLC = 8,
    .ID = 0x250,
    .data = {0x03, 0x14, 0x00, 0x33, (uint8_t)((BATTERY_AH_MAX * BATTERY_NOMINAL_VOLTAGE / 1000000) >> 8), (uint8_t)(BATTERY_AH_MAX * BATTERY_NOMINAL_VOLTAGE / 1000000), 0x03,//{0x03, 0x16, 0x00, 0x66, (uint8_t)((BATTERY_WH_MAX / 100) >> 8), (uint8_t)(BATTERY_WH_MAX / 100), 0x02,
             0x09}};  //3.16 FW , Capacity kWh byte4&5 (example 24kWh = 240)
CAN_frame BYD_290 = {.FD = false,
                     .ext_ID = false,
                     .DLC = 8,
                     .ID = 0x290,
                     .data = {0x0C, 0x66, 0x25, 0x9A, 0x00, 0x00, 0x00, 0x00}};//.data = {0x06, 0x37, 0x10, 0xD9, 0x00, 0x00, 0x00, 0x00}};
...
CAN_frame BYD_3D0_3 = {.FD = false,
                       ...
                       .DLC = 8,
                       .ID = 0x3D0,
                       .data = {0x03, 0x56, 0x4D, 0x00, 0x00, 0x00, 0x00, 0x00}};  //VS
...
void update_values_can_inverter() {  //This function maps all the values fetched from battery CAN to the correct CAN messages
  ...
  //Maximum discharge power allowed (Unit: A+1)
  BYD_150.data.u8[4] = (datalayer.battery.status.Remain_Ah >> 8);
  BYD_150.data.u8[5] = (datalayer.battery.status.Remain_Ah & 0x00FF);
  //Maximum charge power allowed (Unit: A+1)
  BYD_150.data.u8[6] = ( (BATTERY_AH_MAX / 100)>> 8);
  BYD_150.data.u8[7] = ( (BATTERY_AH_MAX / 100)& 0x00FF);
...
void send_can_inverter() {
  ...
  if (currentMillis - previousMillis2s >= INTERVAL_2_S) {
    ...
    transmit_can(&BYD_110, 0);
  }
  // Send 10s CAN Message
  if (currentMillis - previousMillis10s >= INTERVAL_10_S) {
    previousMillis10s = currentMillis;

    transmit_can(&BYD_150, 0);
    transmit_can(&BYD_1D0, 0);
    transmit_can(&BYD_210, 0);
  }
  //Send 60s message
  if (currentMillis - previousMillis60s >= INTERVAL_60_S) {
    previousMillis60s = currentMillis;

    transmit_can(&BYD_190, 0);
  }
}

void send_intial_data() {
  transmit_can(&BYD_250, 0);
  transmit_can(&BYD_290, 0);
  transmit_can(&BYD_2D0, 0);
  transmit_can(&BYD_3D0_0, 0);
  transmit_can(&BYD_3D0_1, 0);
  transmit_can(&BYD_3D0_2, 0);
  transmit_can(&BYD_3D0_3, 0);
}
#endif